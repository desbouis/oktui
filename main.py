...
class OKtui(App):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Horizontal():
            with Vertical(id="sidebar"):
                ...
                self.widget_instances_list = DataTable(
                    header_height=2,
                    show_header=False,
                    cursor_type="row",
                    id="instances-list",
            ...
            with TabbedContent(initial="tab-server-show", id="main-tabbed-content"):
                with TabPane("Server show", id="tab-server-show"):
                    self.widget_content_server_show = RichLog(
                        id="content-server-show",
                        highlight=True,
                        markup=False,
                        auto_scroll=False,
                    )
                    self.widget_content_server_show.border_title = self.initial_labels[
                        "title_server_show"
                    ]
                    yield self.widget_content_server_show
                with TabPane("Console log show", id="tab-console-log-show"):
                    self.widget_content_console_log_show = RichLog(
                        id="content-console-log-show",
                        highlight=True,
                        markup=False,
                        auto_scroll=True,
                    )
                    self.widget_content_console_log_show.border_title = (
                        self.initial_labels["title_console_log_show"]
                    )
                    yield self.widget_content_console_log_show
                with TabPane("Server event list", id="tab-server-event-list"):
                    self.widget_content_server_event_list = RichLog(
                        id="content-server-event-list",
                        highlight=True,
                        markup=False,
                        auto_scroll=False,
                    )
                    self.widget_content_server_event_list.border_title = (
                        self.initial_labels["title_server_event_list"]
                    )
                    yield self.widget_content_server_event_list

        with Horizontal(id="status-bar-set"):
            self.widget_status_bar = Static(
                self.initial_labels["status_bar"], id="status-bar-message"
            )
            yield self.widget_status_bar
            self.widget_release = Static(
                f"{self.app_name} {self.app_release}", id="status-bar-release"
            )
    ...
    def fetch_auth_token(self) -> None:
        ...
        try:
            if not self.auth_token:
                ...
                self.auth_token["auth_args"] = (
                    f"--os-token={self.auth_token['id']} --os-cloud= --os-auth-url=https://auth.cloud.ovh.net/v3 --os-auth-type=token"
                )
    ...
    def load_instances(self, search: str = "") -> None:
        ...
        try:
            ...
            for instance in instances_to_display:
                self.widget_instances_list.add_row(
                    instance["Name"],
                    key=instance["Name"],
            ...
            self.widget_status_bar.update(
                f"{len(instances_to_display)} instances [Last update: {self.last_update}]"
            )
            ...
            self.widget_status_bar.update(
                f"Error: Invalid JSON from OpenStack 'server list': {e}"
            )
            log(f"Error: {e}")
        except Exception as e:
            self.widget_status_bar.update(f"Error: {e}")
    ...
    async def refresh_all(self) -> None:
        ...
        self.widget_content_console_log_show.border_title = self.initial_labels[
            "title_console_log_show"
        ]
        self.widget_content_console_log_show.border_subtitle = ""
        self.widget_content_server_event_list.clear()
        self.widget_content_server_event_list.border_title = self.initial_labels[
            "title_server_event_list"
        ]
        ...
        try:
            if self.selected_instance_name:
                ...
                if tab.active == "tab-server-show":
                    ...
                    self.widget_content_server_show.clear()
                    self.widget_content_server_show.border_title = (
                        "Executing command..."
                    )
                    self.widget_content_server_show.border_subtitle = ""
    ...
    def watch_last_update(self, value: str) -> None:
        ...
        if value:
            self.widget_status_bar.update(f"Instances list last update: {value}")
    ...
    def watch_selected_instance_name(self, value: str) -> None:
        ...
        if value:
            ...
            self.query_one(
                "#main-tabbed-content", TabbedContent
            ).active = "tab-server-show"
            # Execute server show
    ...
    def fetch_server_show(self, value: str) -> None:
        ...
        try:
            ...
            if not self.cache_server_show.get(value):
                ...
                result = subprocess.run(
                    cmd_exec, capture_output=True, text=True, check=True
                )
            ...
            self.widget_content_server_show.border_subtitle = (
                f"Last update: {self.cache_server_show[value]['last_update']}"
            )
            self.widget_content_server_show.write(
                self.cache_server_show[value]["output"]
            )
        except Exception as e:
            self.widget_content_server_show.clear()
    ...
    def fetch_console_log_show(self, value: str) -> None:
        ...
        try:
            ...
            self.widget_content_console_log_show.border_subtitle = (
                f"Last update: {self.cache_console_log_show[value]['last_update']}"
            )
            self.widget_content_console_log_show.write(
                self.cache_console_log_show[value]["output"]
            )
        except Exception as e:
            self.widget_content_console_log_show.clear()
    ...
    def fetch_server_event_list(self, value: str) -> None:
        ...
        try:
            ...
            self.widget_content_server_event_list.border_subtitle = (
                f"Last update: {self.cache_server_event_list[value]['last_update']}"
            )
            self.widget_content_server_event_list.write(
                self.cache_server_event_list[value]["output"]
            )
        except Exception as e:
            self.widget_content_server_event_list.clear()
